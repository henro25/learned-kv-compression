...
Works with GPT‑2‑style and Qwen‑family models.
"""
...
def compress_kv_cache(past, aes, bits=None):
    """Layer‑wise AE compression + optional quantisation (now using reshape)."""
    rebuilt = []
    for (k, v), ae in zip(past, aes):
        B, H, S, D = k.shape
        k_flat, v_flat = k.reshape(-1, D), v.reshape(-1, D)   # ← FIXED
        with torch.no_grad():
            k_lat, v_lat = ae.encoder(k_flat), ae.encoder(v_flat)
            if bits is not None:
                k_lat, v_lat = quantize_tensor(k_lat, bits), quantize_tensor(v_lat, bits)
            k_rec = ae.decoder(k_lat).reshape(B, H, S, D)     # ← FIXED
            v_rec = ae.decoder(v_lat).reshape(B, H, S, D)     # ← FIXED
        rebuilt.append((k_rec, v_rec))
    return DynamicCache.from_legacy_cache(tuple(rebuilt))


def prompt_cache(tokenizer, model, prompt: str):
    enc = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=_max_positions(model)).to(model.device)
    with torch.no_grad():
        out = model(**enc, use_cache=True)
    return enc, out.past_key_values


def top_k_logits(logits: torch.Tensor, k: int = 50):
...
def safe_sample(probs: torch.Tensor):
    ...
    probs = torch.where(denom == 0, torch.full_like(probs, 1.0 / probs.size(-1)), probs / denom)
    return torch.multinomial(probs, num_samples=1)


def continue_text(tokenizer, model, enc, past, *, gen_len=20, top_k=50, temp=1.0, bits=None):
    ids, last = enc.input_ids.clone(), enc.input_ids[:, -1:].to(model.device)
    for _ in range(gen_len):
        with torch.no_grad():
            out = model(input_ids=last, past_key_values=past, use_cache=True)
        logits, past = out.logits[:, -1, :], quantize_past_key_values(out.past_key_values, bits)
        filt = top_k_logits(logits / temp, top_k)
        nxt = safe_sample(torch.softmax(filt, dim=-1))
...
def evaluate_with_kv_quant(model, tokenizer, texts, *, max_length=1024, bits=None):
    ...
    with torch.no_grad():
        for text in tqdm(texts, desc="KV‑cache baseline"):
            inp = tokenizer(text, return_tensors="pt", truncation=True, max_length=max_length).to(model.device)
            ids, mask, past = inp.input_ids, inp.attention_mask, None
            for t in range(ids.size(1) - 1):
                if not mask[0, t + 1]:
                    continue
                out = model(input_ids=ids[:, t : t + 1], past_key_values=past, use_cache=True)
                total_loss += loss_fct(out.logits[:, -1, :], ids[:, t + 1]).item()
                total_tokens += 1
...
def run_benchmark(cfg: dict):
    ...
    model = AutoModelForCausalLM.from_pretrained(cfg["model_name"], torch_dtype=dtype, device_map={"": device}, use_cache=True)
    ...
    for prompt in texts[:5]:
        ...
        ae_out = continue_text(
            tok,
            model,
            enc,
            compress_kv_cache(cache, aes, bits_list[0]),
            gen_len=gen_len,
            top_k=top_k,
            bits=None,
        )
